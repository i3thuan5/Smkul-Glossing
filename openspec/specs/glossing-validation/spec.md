...
切分行與 gloss 行對不起來時 SHALL 分成五類記錄,分類名稱與 `features/切分與glossing對應.feature` 的 Scenario 一致。詞數不同時 SHALL 整行判斷,詞數相同時 SHALL 逐個 token 比切分符號、取第一個對不起來的 token 對:
...
- **THEN** 兩行的 token 數與每個 token 的符號都呼應,通過

#### Scenario: gloss 的詞比切分少

- **WHEN** 比對切分 `nawhani ha-lafin=to ko aro itini`(6 個)與 gloss `因為HA-過夜=完成貌 主格 居住 在這裡`(4 個)
- **THEN** 分類記「gloss 的詞比切分少」

#### Scenario: 切分沒有切,gloss 有切

- **WHEN** 比對某組,切分 token `pitapal` 沒有符號、對應的 gloss token `PI-發現` 有一個 `-`
- **THEN** 分類記「切分沒有切,gloss 有切」,並指出是 `pitapal` 對 `PI-發現`
