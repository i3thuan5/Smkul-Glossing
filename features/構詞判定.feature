Feature: 構詞的判定
  ...
  When 是要判定的那一個詞，Then 是程式目前的判定結果。
  Given 依照《常用構詞標記清單》，只列出「拿掉就會改變判定結果」的那幾列。
  若沒有寫 Given 的 Scenario，代表判定只看切分符號的結構，
  不需要參考《常用構詞標記清單》。
...
Scenario: 3a 剛好一段沒被標成詞綴：清單裡有的算詞綴，剩下的是詞根
  # 例 1（下面用這一組判定）：阿美族/海岸/2_阿索.該拿Aso.Kaynga_劉正州Kacaw_我的成長故事.txt 第 67 組
  #   要判定的詞：Tala-likor-ay=to
  #   族語原文：Talalikoray to ko mihecaan
  #   切分：Tala-likor-ay=to ko miheca-an
  #   glossing：去-以後-實現=完成貌 主格 年-AN
  #   翻譯：學期也往後一年了
  # 例 2：魯凱族/霧台/2.包鳯嬌_Tuku_建築石板屋與傳說故事.txt 第 70 組
  #   要判定的詞：pi-kay=li
  #   族語原文：kikay la yai pikayli kay cukui
  #   切分：kikay la yai pi-kay=li kay cukui
  #   glossing：這 然後 主題標記 放-這=我.屬格 這 桌子
  #   翻譯：這個在桌上的
  # 例 3：排灣族/中排灣/2.廖桂香sakenge_廖進花-tinaqetaq 芋頭粉加花生粉蔬菜飯.txt 第 198 組
  #   要判定的詞：pi-zua
  #   族語原文：pizua ramaljemaljeng a djemu'ul ta qavain,
  #   切分：pi-zua ra<malje>maljeng a dj<em>u'ul ta qava-in
  #   glossing：放-主格.那 <重疊>長輩 連繫詞 <主焦>搗 斜格 糕-受焦
  #   翻譯：長輩們搗芋頭做湯圓用，
  Given 在阿美語常用構詞標記清單中
    | 編號 | 類別   | 詞綴或詞 | 族語範例       | 詞素翻譯              | 備註                                  |
    | 9-79 | 以後   | anoayaw  | anoayaw        | ano-前                | (南勢阿美)                            |
    | 9-17 | 實現   | -ay      | mi-liloc-ay-ay | 主焦-洗澡-實現-名物化 | (南勢阿美) 前-ay表實現，後-ay表名物化 |
    | 4-20 | 完成貌 | =to      | k<om>aen-ay=to | <主焦>吃-實現=完成貌  |                                       |
  When 切分是 "Tala-likor-ay=to"，glossing 是 "去-以後-實現=完成貌"
  Then 判定結果是
    | 形    | 義     | 構詞   |
    | Tala  | 去     | 詞根   |
    | likor | 以後   | 後綴   |
...
Scenario: 2b 重疊：跨過 ~ 的——太魯閣語 q~qlahang
  ...
  # 完全乾淨的例子可以用。
  #
  # 程式目前為什麼會判錯：程式把「~ 後面那一段」直接當成重疊，
  # 從詞根的候選裡剔掉，根本不看 gloss。太魯閣語的重疊是加在
  # 前面的（q~、b~），所以只剩前面那一段可以當詞根，正好判反。
  # 規格上重疊的位置要看 gloss 寫什麼（1e），不能看它在 ~ 的哪一邊。
  #
...
@規格要討論
Scenario: 拉長音的 == 撞到依附詞的 =——霧台魯凱語 ai==au==i
  # 出處：魯凱族/霧台/2.包鳯嬌_Tuku_建築石板屋與傳說故事.txt 第 105 組
  #   要判定的詞：ai==au==i
  #   族語原文：ai~au~i
  #   切分：ai==au==i
  #   glossing：啊伊==啊烏==伊
  #   翻譯：阿伊~阿烏~伊
  #
  # 拉長音的 == 是兩個 = 疊起來的，可是 = 同時也是依附詞的切分符號
  # （霧台魯凱語清單 4-1 =nga、5-1 =ku 等都是）。程式一個字元一個字元
  # 掃過去，看到兩個 = 就切兩刀，把 ai==au==i 切成五段，中間夾出來的
  # 兩段是空字串，四段都判成依附詞。
  #
  # 這不只是這一組的問題：任何語言只要在第二行寫了拉長音的 ==，
  # 都會被這樣切壞。要先決定 == 怎麼跟 = 分開認。
  # Then 寫的是程式目前的輸出，請確認要改成什麼。
  Given 在霧台魯凱語常用構詞標記清單中
    | 編號 | 類別   | 詞綴或詞 | 族語範例    | 詞素翻譯     |
    | 1-4  | 拉長音 | ==       | 不用標      | 不用標       |
    | 3-10 | 完成貌 | =nga     | ma-gwase=nga | 靜態.限定-啞=完成貌 |
    | 4-2  | 我.屬格 | =li     | cayli=li    | 年=我.屬格   |
  When 切分是 "ai==au==i"，glossing 是 "啊伊==啊烏==伊"
  Then 判定結果是
    | 形  | 義   | 構詞     |
    | ai  | 啊伊 | 詞根     |
...
@規格要討論
Scenario: 3c 每一段都被標成詞綴：太魯閣語 s-rngaw
  ...
  #   翻譯：這是母親說的其中之一啟示
  #   為什麼難判斷：「周邊焦」與「說」都在清單裡，程式比 gloss 長短，「周邊焦」比「說」長就判 s 是詞根。可是 rngaw（說）看起來才是詞根。
  #   Then 寫的是程式目前的輸出，請確認對不對。
  Given 在太魯閣語常用構詞標記清單中
    | 編號 | 類別 | 詞綴或詞 | 族語範例 | 詞素翻譯 | 備註 |
    | 4-6  | 周邊焦 | s-       |          |          |      |
...
@規格要討論
Scenario: 3c 每一段都被標成詞綴：阿美語 tayni-ay
  ...
  #   翻譯：我來這裡覺得很好
  #   為什麼難判斷：「來」與「實現」都在清單裡，程式比 gloss 長短，「實現」比「來」長就判 ay 是詞根。可是 tayni（來）看起來才是詞根。
  #   Then 寫的是程式目前的輸出，請確認對不對。
  Given 在阿美語常用構詞標記清單中
    | 編號 | 類別 | 詞綴或詞 | 族語範例 | 詞素翻譯 | 備註 |
    | 4-5  |      | Ø-       | tayni    | 來       |      |
...
@規格要討論
Scenario: 已知會判錯：實詞的意思撞到清單裡的詞綴 gloss
  # 這一條記的是**現在判錯**的情形，Then 寫的是程式目前的
  # 輸出，不是正確答案。修好程式以後要回來改這裡的表格。
  #
  # 共同原因：程式靠「gloss 有沒有在清單裡」認詞綴，
  # 可是實詞的意思有時候剛好就是某個詞綴的 gloss。
  ...
  #   應該要是：vaik（去）才是詞根，-i 是祈使後綴。
  #   為什麼判錯：兩段的 gloss 都在清單裡，程式改比 gloss 長短，「主焦.祈使」比「去」長，就把 i 判成詞根。
  ...
  #   應該要是：mwa（去）才是詞根，a- 是前綴。
  #   為什麼判錯：兩段都算標記，程式比 gloss 長短，「限定」比「去」長，就把 a 判成詞根。
  ...
  #   應該要是：sasu 才是詞根，m-u- 是前綴串。
  #   為什麼判錯：三段都算標記，程式先排除全大寫的 SASU，再比 gloss 長短，就把 u 判成詞根。
  Given 在排灣語常用構詞標記清單中
    | 編號 | 類別      | 詞綴或詞 | 族語範例                 | 詞素翻譯       | 備註 |
...
Scenario: 難判斷：阿美語 na-ni 的 過去 在清單裡、NI 是全大寫
  ...
  #   glossing：好像 名詞標記 過去-NI 在那裡-實現 介系詞 重疊-四處 屬格 部落
  #   翻譯：都是還自各部落移居到這裡的
  #   為什麼難判斷：兩段都算標記——「過去」在清單裡，
  #   NI 是全大寫的不確定標記。程式先排除全大寫，只剩
  #   「過去」，就把 na 判成詞根。可是 ODS 4-27 正好把 na-
  #   登記成「過去」的前綴，照這樣看應該是 ni 才是詞根。
  Given 在阿美語常用構詞標記清單中
    | 編號 | 類別                     | 詞綴或詞 | 族語範例 | 詞素翻譯 | 備註 |
  ...
  # 下面兩條是同一種情形：詞幹每一段的 gloss 都在清單裡，
  # 程式只能比 gloss 長短決定詞根。全語料有 350 個詞是這樣，
  # 其中 199 個就算套用「只寫焦點的是詞綴」也還是分不出來
